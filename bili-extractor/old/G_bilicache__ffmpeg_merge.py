...
if __name__ == '__main__':
    dirs = os.listdir()
    NEW_TARGET = "F:\\Favorites\\bilibili"
    dirs = [directory for directory in dirs if os.path.isdir(directory) and directory != "__pycache__" and (
                (directory+".mp4" not in dirs) and (not os.path.exists(os.path.join(NEW_TARGET, directory+".mp4")))
            )
        ]
    root = os.getcwd()
